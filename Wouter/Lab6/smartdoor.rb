...
state 'closed_unlocked'
  choice {
    ...
    o {
      receive 'unlock', constraint: 'passcode == passcode' 
      send 'invalid_command'
      goto 'closed_unlocked'
    }
    o {
      receive 'lock', constraint: 'passcode < 0000 || passcode > 9999'
      send 'invalid_passcode'
      goto 'closed_unlocked'
    }
    o {
      receive 'lock', constraint: 'passcode >= 0000 && passcode <= 9999', update: 'saved_passcode = passcode'
      send 'locked'
      goto 'closed_locked'}
  }
...
state 'closed_locked'
  choice {
    ...
    o {receive 'unlock', constraint: 'saved_passcode == passcode', update: 'tries = 0'
       send 'unlocked'; goto 'closed_unlocked'}
    o {receive 'unlock', constraint: 'saved_passcode != passcode && tries + 1 > 3', update: 'tries = tries + 1'
       send 'incorrect_passcode'; goto 'lockdown'}
    o {receive 'unlock', constraint: 'saved_passcode != passcode'
       send 'incorrect_passcode'; goto 'closed_locked'}
    o {receive 'unlock', constraint: 'passcode < 0000 || passcode > 9999', update: 'tries = tries + 1'
       send 'invalid_passcode'
       goto 'closed_locked'}
    o {receive 'lock', constraint: 'passcode == passcode'
      send 'invalid_command'; goto 'closed_locked'}
  }
...
state 'opened'
  choice {
    ...
    o {receive 'unlock', constraint: 'passcode == passcode' 
